FASE_3()
{
    ...
    */
    ...
    int mapaValido;
    int back_up[8][7];
    int contador_movimento_3 = 0;

    ///BITMAPS
    BITMAP* buffer = create_bitmap(SCREEN_W, SCREEN_H);
    BITMAP* boneco = load_bitmap("sprites/Personagens/walter_2.bmp", NULL);
    BITMAP* fase_3 = load_bitmap("sprites/Fases/fase-3-facil.bmp", NULL);
    BITMAP* meta = load_bitmap("sprites/Fases/meta.bmp", NULL);
    BITMAP* background = load_bitmap("sprites/Fundos/FUNDO_3.bmp", NULL);
    ...
    while(!exit_program || !key[KEY_ESC])
    {
        ...
        draw_sprite(buffer, fase_3, 30, 55);
        textprintf_ex(buffer, font, 540, 108.5, makecol(51, 51, 51), -1, "%d", contador_movimento_3);
        ...
        if (mapaAcabou)
        {
            contador_movimento_total += contador_movimento_3;
            passou_fase();
        }
        ...
        for (i = 0; i < 8; i++)
        {
            for (k = 0; k < 7; k++)
            {
                ...
                {
                    draw_sprite(buffer, meta, k*50+30, i*50+55);
                }
                if (posicoesAtuais[i][k] % 3 == 0)// chao = boneco
                {
                    draw_sprite(buffer, boneco, k*50+30, i*50+55);
                }
            }
        }
    ...
    destroy_bitmap(fase_3);
    destroy_bitmap(boneco);
    destroy_bitmap(meta);
    destroy_bitmap(buffer);

} END_OF_FUNCTION(FASE_3);
