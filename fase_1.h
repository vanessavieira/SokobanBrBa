int fase_atual;
int contador_movimento_total = 500;
...
FASE_1()
{
    ...
    */
    ...
    int mapaValido;
    int back_up[7][7];
    int contador_movimento_1 = 0;

    ///BITMAPS
    BITMAP* buffer = create_bitmap(SCREEN_W, SCREEN_H);
    BITMAP* boneco = load_bitmap("sprites/Personagens/walter_1.bmp", NULL);
    BITMAP* fase_1 = load_bitmap("sprites/Fases/fase-1-facil.bmp", NULL);
    BITMAP* meta = load_bitmap("sprites/Fases/meta.bmp", NULL);
    BITMAP* background = load_bitmap("sprites/Fundos/FUNDO_1.bmp", NULL);
    ...
    while(!exit_program || !key[KEY_ESC])
    {
        ...
        draw_sprite(buffer, fase_1, 30, 70);
        textprintf_ex(buffer, font, 540, 108.5, makecol(51, 51, 51), -1, "%d", contador_movimento_1);
        ...
        if (mapaAcabou)
        {
            contador_movimento_total = contador_movimento_1;
            passou_fase();
        }
        ...
        for (i = 0; i < 7 && !stop; i++)
        {
            for (k = 0; k < 7 && !stop; k++)
            {
                if (posicoesAtuais[i][k] % 42 == 0) //pedra em cima do boneco
                {
